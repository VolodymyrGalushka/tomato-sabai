// <% nvram("vpn_service,pptp_on,ovpn_on,vpn_user,vpn_pass,vpn_fix,vpn_server,pptp_mppe,pptp_stateful,pptp_defgw,ovpn_type,ovpn_file,ovpn_si,ovpn_up,ovpn_dn,ovpn_cf,gw_def,gw_1,gw_2,gw_3,ac_on,ac_ip,dhcpd_static,dhcp_start,dhcp_num,dhcpd_startip,dhcpd_endip,reg_info"); %>
var msg_sabaivpn = '<% sabaaiMSG() %>';
var uinf=((nvram.reg_info=='')?',,':nvram.reg_info).split(',');

// // /*BEGIN SABAIVPN ACTIVE JAVASCRIPT */
...
function update_VPN_stats(vpns){ if( (vpns!=null) && (vpns!=undefined) ){ eval(vpns);
 if( (vpnstats.ovpn_status==1)||(vpnstats.pptp_status==1) ){ show_VPN_status(vpnstats);
  if(E('m_vpn_status').innerHTML!='Connected'){ setTimeout( redo_VPN_stats , 5000 ); }
  else{ if(window.VPN_test==null){ window.VPN_test = 10; }else if(window.VPN_test-->0){ setTimeout(redo_VPN_stats,10000); }else{ setTimeout(redo_VPN_stats,60000) } }
 }
}else{ redo_VPN_stats(); } }

if(nvram.ovpn_on==1||nvram.pptp_on==1){ redo_VPN_stats(); }else{ setTimeout(redo_VPN_stats,30000); }
